# ─────────────────────────────────────────────────────────────────────────────
#  EXO BRAIN — Windows Setup (PowerShell)
...
'@ -Name 'VTConsole' -Namespace 'ExoBrainVT' -ErrorAction Stop
    $h = [ExoBrainVT.VTConsole]::GetStdHandle(-11)
    $mode = 0
    [ExoBrainVT.VTConsole]::GetConsoleMode($h, [ref]$mode) | Out-Null
    [ExoBrainVT.VTConsole]::SetConsoleMode($h, $mode -bor 0x0004) | Out-Null
} catch {
...
if (-not (Test-Path "$scriptDir\CLAUDE.md")) {
    ...
    if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
        ...
        Write-Host "    2. git clone https://github.com/bsy/exo-brain.git"
        Write-Host "    3. cd exo-brain"
        Write-Host "    4. powershell -ExecutionPolicy Bypass -File setup.ps1"
        exit 1
    }
    $bootstrapDir = Join-Path ([System.IO.Path]::GetTempPath()) "exo-brain-setup"
    Remove-Item $bootstrapDir -Recurse -Force -ErrorAction SilentlyContinue
    git clone --depth=1 https://github.com/bsy/exo-brain.git $bootstrapDir *>$null
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  ${Red}ERROR${Reset}  Could not clone repo. Check your internet connection."
        Write-Host "  Alternative: download from https://github.com/bsy/exo-brain"
        exit 1
    }
...
"@
Write-Host "${Reset}"
Write-Host "  Obsidian + Claude Code - Your AI-powered exo brain"
Write-Host ""
Write-Host "  ====================================================="
Write-Host ""
...
if (-not (Test-Path "$scriptDir\requirements.txt")) {
    ...
    Write-Host "        After cloning: cd exo-brain, then rerun:"
...
Write-Host ""
Write-Host "  Where should your exo brain live?"
Write-Host "  ${Dim}Press Enter for default: $env:USERPROFILE\exo-brain${Reset}"
Write-Host "  ${Dim}(e.g. C:\Users\YourName\Documents\MyVault -- quotes OK)${Reset}"
$vaultInput = Read-Host "  Vault path"
if (-not $vaultInput) { $vaultInput = "$env:USERPROFILE\exo-brain" }
...
if ($copyErrors -gt 0) {
    ...
    Write-Host "        Try: git clone https://github.com/bsy/exo-brain.git"
    Write-Host "        Then: cd exo-brain && powershell -ExecutionPolicy Bypass -File setup.ps1"
}
