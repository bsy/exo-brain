...
# ─────────────────────────────────────────────────────────────────────────────
#  EXO BRAIN — One-command setup for Obsidian + Claude Code
# ─────────────────────────────────────────────────────────────────────────────
...
if [ ! -f "$SCRIPT_DIR/CLAUDE.md" ]; then
  echo "Downloading Exo Brain repo..."
  BOOTSTRAP_DIR="$(mktemp -d)"
  if ! git clone --depth=1 https://github.com/bsy/exo-brain.git "$BOOTSTRAP_DIR" &>/dev/null; then
    echo "Error: Could not clone repo. Check your internet connection."
    echo "Alternative: download the ZIP from https://github.com/bsy/exo-brain"
    echo "             unzip it, cd into it, and run: bash setup.sh"
    exit 1
  fi
  # Pass cleanup marker so the re-exec'd script can clean up
  EXO_BRAIN_BOOTSTRAP_DIR="$BOOTSTRAP_DIR" exec bash "$BOOTSTRAP_DIR/setup.sh"
fi

# Clean up bootstrap temp dir if we were re-exec'd from the one-liner
if [ -n "$EXO_BRAIN_BOOTSTRAP_DIR" ] && [ -d "$EXO_BRAIN_BOOTSTRAP_DIR" ]; then
  trap 'rm -rf "$EXO_BRAIN_BOOTSTRAP_DIR"' EXIT
fi
...
ASCII
echo -e "${RESET}"
echo -e "${DIM}  Obsidian + Claude Code · Your AI-powered exo brain${RESET}"
echo ""
echo -e "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
if command -v python3 &>/dev/null; then
  ...
  if "$VENV_DIR/bin/pip" install -q -r "$SCRIPT_DIR/requirements.txt" 2>/dev/null; then
    ...
    echo -e "     python3 -m venv ~/.exo-brain-venv && ~/.exo-brain-venv/bin/pip install -r requirements.txt"
  fi
else
...
echo ""
echo -e "  Where should your exo brain live?"
echo -e "  ${DIM}Press Enter for default: ~/exo-brain${RESET}"
echo -e "  ${DIM}(e.g. ~/Documents/MyVault or /Users/yourname/Notes)${RESET}"
read -rp "  Vault path: " VAULT_PATH
VAULT_PATH="${VAULT_PATH:-$HOME/exo-brain}"
...
if [ "$REAL_VAULT" = "$REAL_SCRIPT" ]; then
  echo -e "  ${ORANGE}⚠${RESET}  Vault can't be the same folder as the repo. Using ~/exo-brain instead."
  VAULT_PATH="$HOME/exo-brain"
fi
...
if [ -n "$IMPORT_FOLDER" ] && [ -d "$IMPORT_FOLDER" ]; then
  if [ "$PIP_OK" = false ]; then
    ...
    if "$HOME/.exo-brain-venv/bin/python3" "$VAULT_PATH/scripts/process_docs_to_obsidian.py" \
